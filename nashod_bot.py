...
def about(chat_id):
    text = " سلام 🛂%0A((نشد بات...☁️)) یه بخش از سرور مارتینهⓂ️%0Aاین ربات ربات یه ایده از یکی از دوستای مارتین بود و پاییز ۱۴۰۰ ساخته شد%0Aما همیشه آنلاین ؛ در دسترس شما هستیم😅"
    send_message(text, chat_id)

def help(chat_id):
    text = ("من اینجام که کمکت کنم!%0A برای دریافت راهنمایی در مورد هر موضوع، کافیه دستور آبی رنگی که مقابل اون سوال هست رو لمس کنی:%0A%0A👨‍💻 پشتیبانی ربات : %0A @MifoSupport")
